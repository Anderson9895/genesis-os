...
drop policy if exists "Users can insert their own TikTok settings" on public.tiktok_settings;
...
create index if not exists tiktok_webhook_events_user_idx
  on public.tiktok_webhook_events (user_id, received_at desc);
