...
export function vaultConfigured() {
  ...
  const hasKey = Boolean(encryptionKeyBuffer())
  return {
    configured: hasToken && hasKey,
    hasStoredToken: hasToken,
    encryptionAtRest: hasKey,
    note: hasToken && !hasKey
      ? 'A token is set but encryption-at-rest is not; add a TIKTOK_TOKEN_ENCRYPTION_KEY of at least 32 characters before live use.'
      : null,
  }
}
